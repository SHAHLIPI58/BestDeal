...
public class PhoneList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
		     allphones = MySqlDataStoreUtilities.getPhones(); //getPhones()
		}
		catch(Exception e)
		{
...
Utilities utility = new Utilities(request,pw);
		...
		for(Map.Entry<String, Phone> entry : hm.entrySet())
		{
			...
			System.out.print("PhoneList------------->"+phone.getDiscount());
			...
			pw.print("<li><form method='post' action='Cart'>" +
					...
					"<input type='hidden' name='name1' value='"+phone.getName()+"'>"+
					"<input type='hidden' name='type' value='phones'>"+
					"<input type='hidden' name='maker' value='"+CategoryName+"'>"+
					//phone.getDiscount()
					"<input type='hidden' name='discount' value='"+phone.getDiscount()+"'>"+
